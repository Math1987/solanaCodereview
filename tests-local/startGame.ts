...
describe("create and start game", () => {
  ...
  before( async () => {
    ...
    const tx = await provider.connection.requestAirdrop(game.publicKey, anchor.web3.LAMPORTS_PER_SOL );
    await provider.connection.confirmTransaction(tx, "confirmed");
    const balance = await provider.connection.getBalance(game.publicKey);
